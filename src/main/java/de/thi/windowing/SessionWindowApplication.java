...
public class SessionWindowApplication {
    private static final Logger log = LoggerFactory.getLogger(SessionWindowApplication.class);

    //0. 定义一些常量
    private static final String BOOTSTRAP_SERVERS = "localhost:19092";
    private static final String APPLICATION_ID = "session_window_app_id";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        KStream<String, NetTraffic> ks0 = builder.stream(SALES_STATS_SOURCE_TOPIC, Consumed.with(Serdes.String(), JsonSerdes.NetTraffic())
                .withName("session-window-source-processor")
                .withOffsetResetPolicy(AutoOffsetReset.latest())
        );
        ks0.groupBy((k, v) -> v.getRemoteAddr(), Grouped.with(Serdes.String(), JsonSerdes.NetTraffic()))

                .windowedBy(SessionWindows.ofInactivityGapWithNoGrace(Duration.ofMinutes(1)))
                .count(Named.as("session-window-count-processor"), Materialized.as("session-window-count-store"))
                .suppress(Suppressed.untilWindowCloses(Suppressed.BufferConfig.unbounded()))
                .toStream()
                    .peek((k, v) -> log.info("用户 :{} 在[{} --- {}]内访问了网站 {} 次", k.key(), k.window().start(), k.window().end(), v), Named.as("peek-processor"));
